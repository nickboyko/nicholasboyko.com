...
function updateItemToggleTheme() {
    ...
    const darkSheet = document.querySelector('link[href$="syntax-dark.css"]');
    const lightSheet = document.querySelector('link[href$="syntax-light.css"]');

    if (mode === "dark") {
        if (darkSheet) darkSheet.media = "all";
        if (lightSheet) lightSheet.media = "not all";
    } else {
        if (darkSheet) darkSheet.media = "not all";
        if (lightSheet) lightSheet.media = "all";
    }
