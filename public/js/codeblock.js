...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('a.zola-anchor code').forEach(function(code) {
        const parentLink = code.closest('a.zola-anchor'); // Specific to zola-anchor links
        if (parentLink) {
            parentLink.addEventListener('mouseenter', function() {
            code.classList.add('no-transition');
            void code.offsetWidth; // Force reflow
            });
            parentLink.addEventListener('mouseleave', function() {
            code.classList.add('no-transition');
            void code.offsetWidth; // Force reflow
            setTimeout(() => code.classList.remove('no-transition'), 20);
            });
        }
    });
});
